    ...
    <v-row justify="center">
      <v-col cols="12" sm="10" md="8" lg="6">
        <v-card class="mt-4 elevation-2">
          <v-card-title class="text-h5 pa-4">{{ t('product.title.add') }}</v-card-title>
          <v-divider/>
          <v-card-text>
            <vrd-product-form :product="product" v-model:isError="isError" :loading="isSubmitting" 
              @submit="save" 
...
<script setup lang="ts">
    ...
    async function save(productData: Product) {
